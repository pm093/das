...
<div class="n68137">{{if  $xml->getItem("page/client_files/@mode")  == (string) 'to_download'}}
...
<span class="n68137_label">
                                {{$txt['71387::68137_label']}}
                            </span></div>
<div class="n68137_description">
<div class="n68137_description_cms">{{$xml->getItemEscape("/shop/page/cms_txt/cms[@type='to_download']")}}{{$xml->getItemEscape("/shop/page/cms_txt/cms[@type='not_available']")}}</div></div>
<div class="n68137_files">{{foreach from=$xml->getList("/shop/page/client_files/product_data") item=loop262}}
<div class="n68137_file">
<div class="n68137_files_image">
<img class="n68137_img" src="{{$loop262->getItemEscape("@icon")}}" alt="{{$loop262->getItemEscape("product_attachments/attachment/@file_name")}}"></img></div>
<div class="n68137_files_description">
<a class="n68137_desc_name" href="{{$loop262->getItemEscape("@link")}}">{{$loop262->getItemEscape("@name")}}</a>
...
<a class="n68137_desc_value" href="{{$loop262->getItemEscape("@order_link")}}">{{$loop262->getItemEscape("@order_sn")}}</a></span></div>
<div class="n68137_files_description_availability">
<span class="n68137_desc_label">{{$txt['71387::68137_availability']}}</span>
<span class="n68137_desc_value">{{if  $loop262->getItem("@available")  == (string) 'y'}} 
                                                        {{$txt['71387::68137_file_available']}}
                                                    {{else}}
                                                        {{$txt['71387::68137_file_unavailable_pay']}}
                                                    {{/if}}</span></div></div>{{if  $loop262->getItem("@available")  == (string) 'y'}}
<button{{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}n68137_files_download{{/capture}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}btn-small n68137_files_download{{/capture}}  class="{{$classAttributeTmp4}}">
{{$txt['71387::68137_download']}}</button>{{else}}
<a{{assign "classAttributeTmp5" ""}}{{capture name="classAttributeTmp5" assign="classAttributeTmp5"}}n68137_files_pay{{/capture}}{{capture name="classAttributeTmp5" assign="classAttributeTmp5"}}n68137_files_pay btn-small{{/capture}} href="{{$loop262->getItemEscape("@order_link")}}"  class="{{$classAttributeTmp5}}">{{$txt['71387::68137_pay']}}</a>{{/if}}
<div class="clearDiv"></div>
<div class="n68137_popup">
<div class="n68137_close_popup"></div>
<div{{assign "classAttributeTmp6" ""}}{{capture name="classAttributeTmp6" assign="classAttributeTmp6"}}n68137_label_popup{{/capture}}{{capture name="classAttributeTmp6" assign="classAttributeTmp6"}}big_label n68137_label_popup{{/capture}}  class="{{$classAttributeTmp6}}">
                                            {{$txt['71387::68137_label_popup']}}
                                        </div>
<div class="n68137_description_popup">
                                            {{$txt['71387::68137_cms_popup']}}
                                            
<div class="n68137_description_cms_popup">{{$loop262->getItemEscape("/shop/page/cms_txt/cms[@type='toplayer']")}}</div></div>{{foreach from=$loop262->getList("product_attachments/attachment") item=loop315}}
<div class="n68137_files_description_popup">
<div class="n68137_desc_popup">
<div class="n68137_desc_name_popup">{{if  $loop315->getItem("@name") and  $loop315->getItem("@name")  neq (string) ''}}
<span class="n68137_desc_name_popup">{{$loop315->getItemEscape("@name")}}</span>{{/if}}{{if  $loop315->getItem("@size_mb") and  $loop315->getItem("@size_mb")  neq (string) '' and  $loop315->getItem("@size_mb")  gt (string) 0}}
<span class="n68137_desc_size_popup">{{$txt['71387::68137_desc_size_popup_bracket_open']}}{{$loop315->getItemEscape("@size_mb")}}{{$txt['71387::68137_desc_size_popup_bracket_close']}}</span>{{/if}}</div>
<div class="n68137_desc_availability_popup">
                                                    {{$txt['71387::68137_available']}}
                                                    {{if  $loop315->getItem("@unlimited") == (string)'true'}}
                                                            {{$txt['71387::68137_unlimited']}}
                                                            {{if  $loop315->getItem("@expiration_date")}}
                                                                {{$txt['71387::68137_until']}}
                                                                {{$loop315->getItemEscape("@expiration_date")}}{{/if}}{{elseif ( $loop315->getItem("@unlimited") == (string)'false') and !( $loop315->getItem("@downloads_limit"))}}
                                                            {{$txt['71387::68137_unlimited']}}
                                                            {{if  $loop315->getItem("@expiration_date")}}
                                                                {{$txt['71387::68137_until']}}
                                                                {{$loop315->getItemEscape("@expiration_date")}}{{/if}}{{else}}{{$loop315->getItemEscape("@downloads_left")}}{{if  $loop315->getItem("@downloads_left")  == (string) 1}}
                                                                    {{$txt['71387::68137_time']}}
                                                                {{else}}
                                                                    {{$txt['71387::68137_times']}}
                                                                {{/if}}{{if  $loop315->getItem("@expiration_date")}}
                                                                {{$txt['71387::68137_until']}}
                                                                {{$loop315->getItemEscape("@expiration_date")}}{{/if}}{{/if}}</div></div>
<a{{assign "classAttributeTmp7" ""}}{{capture name="classAttributeTmp7" assign="classAttributeTmp7"}}n68137_files_download_popup{{/capture}}{{capture name="classAttributeTmp7" assign="classAttributeTmp7"}}btn-small n68137_files_download_popup{{/capture}} href="{{$loop315->getItemEscape("@download_link")}}"  class="{{$classAttributeTmp7}}">
<span class="n68137_button_label_popup">{{$txt['71387::68137_button_label_popup']}}
<span class="n68137_button_value_popup">{{$loop315->getItemEscape("@file_extension")}}</span></span></a>
<div class="clearDiv"></div></div>{{/foreach}}</div></div>{{/foreach}}</div></div>{{elseif  $xml->getItem("page/client_files/@mode")  == (string) 'not_available'}}
<div class="n68137_notdownloadable">
<div{{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}n68137_label{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}big_label n68137_label{{/capture}}  class="{{$classAttributeTmp3}}">
<span class="n68137_label">
                                {{$txt['71387::68137_label_not_available']}}
                            </span></div>
<div class="n68137_description">
<div class="n68137_description_cms"></div></div>
<div class="n68137_files">{{foreach from=$xml->getList("/shop/page/client_files/product_data") item=loop386}}
<div class="n68137_file">
<div class="n68137_files_image">
<img class="n68137_img" src="{{$loop386->getItemEscape("@icon")}}" alt="{{$loop386->getItemEscape("product_attachments/attachment/@file_name")}}"></img></div>
<div class="n68137_files_description">
<a class="n68137_desc_name" href="{{$loop386->getItemEscape("@link")}}">{{$loop386->getItemEscape("@name")}}</a>
<div class="n68137_files_description_order">
<span class="n68137_desc_label">{{$txt['71387::68137_order']}}</span>
<a class="n68137_desc_value" href="{{$loop386->getItemEscape("@order_link")}}">{{$loop386->getItemEscape("@order_sn")}}</a></div>
<div class="n68137_files_description_availability">
<span class="n68137_desc_label">{{$txt['71387::68137_availability']}}</span>
<span class="n68137_desc_value">{{if  $loop386->getItem("product_attachments/@downloads_left")  == (string) '0'}} 
                                                        {{$txt['71387::68137_file_depleted']}}
                                                    {{else}}
                                                        {{$txt['71387::68137_file_unavailable']}}
                                                    {{/if}}</span></div></div>{{if  $loop386->getItem("@available") == (string)'y'}}
<a{{assign "classAttributeTmp9" ""}}{{capture name="classAttributeTmp9" assign="classAttributeTmp9"}}n68137_files_buy{{/capture}}{{capture name="classAttributeTmp9" assign="classAttributeTmp9"}}btn-small n68137_files_buy{{/capture}} href="{{$loop386->getItemEscape("@link")}}"  class="{{$classAttributeTmp9}}">{{$txt['71387::68137_buy']}}</a>{{/if}}
<div class="clearDiv"></div></div>{{/foreach}}</div></div>{{/if}}</div>
<script>

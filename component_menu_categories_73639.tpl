...
<li class="level_1 hidden-phone" id="level_1_position_{{$loop42@iteration}}">{{if  $loop42->getItem("@link") and  $loop42->getItem("@link")  neq (string) '##'}}{{if  $loop42->getItem("item")}}{{literal}}
...
<ul class="level_2">{{foreach from=$loop42->getList("item") item=loop82}}
<li class="level_2">
...
<li class="level_3">
<a{{assign "hrefAttributeTmp3" ""}}{{capture name="hrefAttributeTmp3" assign="hrefAttributeTmp3"}}##{{/capture}} class="level_3" {{if $loop99->getItem("@link")}}{{capture name="hrefAttributeTmp3" assign="hrefAttributeTmp3"}}{{$loop99->getItemEscape("@link")}}{{/capture}}{{/if}}{{if $loop99->getItem("@target")}} target="{{$loop99->getItemEscape("@target")}}" {{/if}}  href="{{$hrefAttributeTmp3}}">{{$loop99->getItem("@name")}}</a></li>{{/foreach}}</ul>{{/if}}</li>{{/foreach}}</ul>{{/if}}</li>{{/foreach}}</ul></div>

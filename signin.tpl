...
<div{{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}signin_cms_sub{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}signin_cms_sub cm{{/capture}}  class="{{$classAttributeTmp3}}">{{$xml->getItem("page/text/body")}}</div></div>{{/if}}
<!--Social networking - logowanie (signin_social_networking, 68380.1)-->
{{if  $xml->getItem("/shop/fn/mode_popup")}}{{/if}}{{if !( $xml->getItem("login_options/service[@connected='true']"))}}{{if count( $xml->getList("login_options/service")) gt (string)1}}
<div id="client_new_social">
<h2 class="{{$txt['68380::n56203_label_class1']}}">
                        {{$txt['68380::n56203_konto']}}
                    </h2>
<div class="client_new_social_sub">{{if !( $xml->getItem("page/client-data/@allow_register")  == (string) 'false')}}
<p>{{$txt['68380::n56203_konto1']}}</p>{{/if}}{{if count( $xml->getList("login_options/service")) gt (string)1}}
<ul>{{foreach from=$xml->getList("login_options/service[not(@name='shop_register')]") item=loop265}}
<li>{{if  $loop265->getItem("@login_url")}}
<a href="{{$loop265->getItemEscape("@login_url")}}">
<span class="social_button_txt">
<span class="social_login_label">{{$txt['68380::56478_009za']}}</span>{{$loop265->getItemEscape("@name")}}</span>{{if  $loop265->getItem("@gfx_big")}}
<img alt="{{$loop265->getItemEscape("@name")}}" src="{{$loop265->getItemEscape("@gfx_big")}}"></img>{{/if}}</a>{{else}}
<strong rel="{{$loop265->getItemEscape("@name")|replace:' ':'_'}}">
<span class="social_button_txt">
<span class="social_login_label">{{$txt['68380::56478_009za']}}</span>{{$loop265->getItemEscape("@name")}}</span>{{if  $loop265->getItem("@gfx_big")}}
<img alt="{{$loop265->getItemEscape("@name")}}" src="{{$loop265->getItemEscape("@gfx_big")}}"></img>{{/if}}</strong>{{/if}}</li>{{literal}} {{/literal}}{{/foreach}}</ul>{{foreach from=$xml->getList("login_options/service[not(@name='shop_register') and not(@login_url)]") item=loop295}}
<form action="login.php" method="post" style="display:none;" id="social_networking_{{$loop295->getItemEscape("@name")|replace:' ':'_'}}" {{if $loop295->getItem("@form_action")}} action="{{$loop295->getItemEscape("@form_action")}}" {{/if}}>
<input type="hidden" name="openid_serviceid" value="{{$loop295->getItemEscape("@id")}}"></input>{{if  $loop295->getItem("@name") == (string)'OpenId'}}
<label>{{$loop295->getItemEscape("@name")}}
<span>{{$txt['68380::56478_009oiu']}}</span></label>{{/if}}{{if !( $loop295->getItem("@name") == (string)'OpenId')}}
<label>{{$loop295->getItemEscape("@name")}}
<span>{{$txt['68380::56478_009olo']}}</span></label>{{/if}}
<input id="signin_login_url" type="text" name="openid_url" size="160"/>
<button type="submit" class="signin_button btn-small {{$loop295->getItemEscape("@name")}}">
                                        {{$txt['68380::56478_log']}}
                                    </button></form>{{/foreach}}{{/if}}</div></div>{{/if}}{{/if}}
...
<div class="signin-form_box_sub">{{if  $xml->getItem("page/actions/client_once")}}
<h2 class="big_label">
                        {{$txt['67949::56478_001']}}
                    </h2>
<p class="registration_desc">{{$txt['67949::56478_002']}}</p>{{if  $xml->getItem("page/actions/client_register")}}
<a{{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}signin-form_register{{/capture}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}btn signin-form_register{{/capture}} href="{{$xml->getItemEscape("page/actions/client_register/@url")}}"  class="{{$classAttributeTmp4}}">
                            {{$txt['67949::56478_003b']}}
                        </a>
			{{$txt['67949::56478_001signin_register3txt']}}			
                    {{/if}}{{if  $xml->getItem("page/actions/client_once")}}
<a{{assign "classAttributeTmp5" ""}}{{capture name="classAttributeTmp5" assign="classAttributeTmp5"}}signin-form_once{{/capture}}{{capture name="classAttributeTmp5" assign="classAttributeTmp5"}}btn signin-form_once{{/capture}} href="{{$xml->getItemEscape("page/actions/client_once/@url")}}"  class="{{$classAttributeTmp5}}">
                            {{$txt['67949::56478_003']}}
                        </a>{{/if}}{{else}}
<h2 class="big_label">
                        {{$txt['67949::56478_004']}}
                    </h2>
<p class="registration_desc">{{$txt['67949::56478_005']}}</p>
<a{{assign "classAttributeTmp6" ""}}{{capture name="classAttributeTmp6" assign="classAttributeTmp6"}}signin-form_register2{{/capture}} href="/client-new.php?register"{{capture name="classAttributeTmp6" assign="classAttributeTmp6"}}btn signin-form_register2{{/capture}}  class="{{$classAttributeTmp6}}">
                        {{$txt['67949::56478_006']}}                
                    </a>{{/if}}</div></div>
...
<p>{{$txt['67949::56478_008']}}</p>
<div class="signin-form_select">
<h3>{{$txt['67949::56478_007login']}}</h3>
<input id="signin-form_radio1" type="radio" value="signin-form_box_sub_1" checked="checked" name="login_form1"/>
<label for="signin-form_radio1">{{$txt['67949::56478_007l1']}}</label>
<input id="signin-form_radio2" type="radio" value="signin-form_box_sub_2" name="email_form1"/>
<label for="signin-form_radio2">{{$txt['67949::56478_007l2']}}</label></div>
<form action="/signin.php" method="post">
<input type="hidden" name="operation" value="login"/>
<div class="signin_inputs">
<label>{{$txt['67949::56478_009']}}</label>
<input autocomplete="off" id="signin_login_input" class="signin_login_input" type="text" name="login" size="20"/></div>
<div class="signin_inputs">
<label>{{$txt['67949::56478_010']}}</label>
<input autocomplete="off" id="signin_pass_input" class="signin_pass_input" type="password" name="password" size="15"/></div>
<button{{assign "classAttributeTmp7" ""}}{{capture name="classAttributeTmp7" assign="classAttributeTmp7"}}signin_button{{/capture}} type="submit"{{capture name="classAttributeTmp7" assign="classAttributeTmp7"}}btn signin_button{{/capture}}  class="{{$classAttributeTmp7}}">
                      {{$txt['67949::56478_log']}}
                </button>
<a class="password_reminder_signin" href="#">{{$txt['67949::65718_remindpassword']}}</a></form>
            {{$txt['67949::56478_011']}}
        </div>
<div class="signin-form_box_sub" id="signin-form_box_sub_2">
<h2 class="big_label">
                {{$txt['67949::56478_007rec']}}
            </h2>
<p class="reminder_desc">{{$txt['67949::51100_001']}}</p>
<div class="signin-form_select">
<h3>{{$txt['67949::56478_007login']}}</h3>
<input id="signin-form_radio3" type="radio" value="signin-form_box_sub_1" name="login_form2"/>
<label for="signin-form_radio3">{{$txt['67949::56478_007l1']}}</label>
<input id="signin-form_radio4" type="radio" value="signin-form_box_sub_2" name="email_form2" checked="checked"/>
<label for="signin-form_radio4">{{$txt['67949::56478_007l2']}}</label></div>
...
<div class="signin_inputs">
<label class="reminder_label_signin">{{$txt['67949::51100_007']}}</label>
<input id="signin_pass_recover_input" type="text" name="email"/>
<button{{assign "classAttributeTmp8" ""}}{{capture name="classAttributeTmp8" assign="classAttributeTmp8"}}signin_password-recover{{/capture}} type="submit"{{capture name="classAttributeTmp8" assign="classAttributeTmp8"}}btn-small signin_password-recover{{/capture}}  class="{{$classAttributeTmp8}}">
                     {{$txt['67949::51100_003']}}
                </button></div></form>
<a class="password_back_signin" href="#">{{$txt['67949::65718_011bis']}}</a>
            {{$txt['67949::56478_011bis']}}
        </div></div></div>{{if  $xml->getItem("/shop/page/password-recover")}}
<span></span>{{/if}}</div></div>

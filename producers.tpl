...
<!--Lista producentów (producers, 55492.1)-->

<div class="header_producers">
<h1 class="big_label">
                   {{$txt['55492::40013_000producers']}}
              </h1></div>
<div class="links_producers">
<div class="links_producers_main">
<div class="links_producers_sub">
<table class="producers_list">{{foreach from=$xml->getList("page/producers-list/firm") item=loop243}}{{if ( $loop243@iteration -1) % 4  == (string) 0}}{{literal}}
<tr>{{/literal}}{{/if}}
<td style="width:25%;" class="producers_list">
<a class="n55492_links" href="{{$loop243->getItemEscape("@link")}}" title="{{$loop243->getItemEscape("@name")}}">{{if  $loop243->getItem("@icon")}}
<img class="links_producers" src="{{$loop243->getItemEscape("@icon")}}" alt="{{$loop243->getItem("@name")}}"></img>{{$txt['55492::40013_000br']}}
                            {{/if}}
<span class="n55492_prod_name">{{$loop243->getItem("@name")}}</span></a>
<span class="n55492_prods_amount">
                            {{$txt['55492::40013_000']}}
                            {{$loop243->getItemEscape("@products")}}{{$txt['55492::40013_001']}}
                        </span></td>{{if ( $loop243@iteration -1) % 4  == (string) (4 - 1) or  $loop243@iteration  == (string) $loop243@total }}{{literal}}</tr>{{/literal}}{{/if}}{{/foreach}}</table></div></div></div></div></div>

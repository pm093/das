...
<div{{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}products_requests_cms_label{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}products_requests_cms_label big_label{{/capture}}  class="{{$classAttributeTmp3}}">
<span class="products_requests_cms_label">
                    {{$txt['58581::products_requests_cms_label']}}
            </span></div>
<div class="products_requests_cms">
<div class="products_requests_cms_sub">{{$xml->getItem("page/text/body")}}</div></div>{{/if}}
<!--Produkty zaproponowane - lista (products_requests_list, 58582.1)-->

<div class="products_requests_list">
<div{{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}products_requests_list_label{{/capture}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}products_requests_cms_label big_label{{/capture}}  class="{{$classAttributeTmp4}}">
<span class="products_requests_list_label">
                    {{$txt['58582::products_requests_list_label']}}
            </span></div>
<div class="products_requests_list_sub">
<table class="products_requests_list" cellspacing="0" cellpadding="0">{{foreach from=$xml->getList("page/products_requests/item") item=loop291}}
<tr class=" {{if $loop291@iteration % 2 == (string) 0}}products_requests_list_itemp{{/if}}{{if $loop291@iteration % 2 == (string) 1}}products_requests_list_itemn{{/if}} ">
<td class="products_requests_list_info1">{{if  $loop291->getItem("@name")}}
<div class="products_requests_list_name">
<span class="products_requests_list_name">{{$loop291->getItemEscape("@name")}}</span></div>{{/if}}{{if  $loop291->getItem("@desc")}}
<div class="products_requests_list_desc">{{$loop291->getItemEscape("@desc")}}</div>{{/if}}</td>
<td class="products_requests_list_info2">
<div class="products_requests_list_date">
<span class="products_requests_list_date_title">{{$txt['58582::products_requests_list_date_title']}}</span>
<span class="products_requests_list_date_val">{{$loop291->getItemEscape("@date")}}</span></div>
<a class="products_requests_list_remlink" href="{{$loop291->getItemEscape("@remove_link")}}" title="{{$txt['58582::products_requests_list_remlink_title']}}">
                                {{$txt['58582::products_requests_list_remlink_txt']}}
                            </a></td></tr>{{/foreach}}</table>
<div class="products_requests_add">
                {{$txt['58582::products_requests_list_add']}}
                
<a class="products_requests_add" href="{{$xml->getItemEscape("page/action/noproduct/@url")}}" title="{{$txt['58582::products_requests_list_addlink_title']}}">
                    {{$txt['58582::products_requests_list_addlink_txt']}}</a>{{$txt['58582::products_requests_list_add2']}}
            </div></div></div></div></div>

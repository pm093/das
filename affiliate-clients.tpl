...
<div{{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}affiliate_clients_label{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}big_label affiliate_clients_label{{/capture}}  class="{{$classAttributeTmp3}}">
<span class="affiliate_clients_label">
            {{$txt['59424::affiliate_clients_label']}}
        </span></div>{{if count( $xml->getList("page/clients_list/login"))}}
<div class="affiliate_clients_desc">
<p class="affiliate_clients_desc">{{$txt['59424::affiliate_clients_desc']}}</p></div>
<div class="affiliate_clients">
<div class="affiliate_clients_sub">
<table{{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}affiliate_clients{{/capture}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}ui-responsive table-stroke ui-table ui-table-reflow affiliate_clients{{/capture}}  class="{{$classAttributeTmp4}}">
<thead>
<tr>
<th id="affiliate_clients_th1">{{$txt['59424::affiliate_clients_th1']}}</th>
<th id="affiliate_clients_th3">{{$txt['59424::affiliate_clients_th3']}}</th></tr></thead>
<tbody>{{foreach from=$xml->getList("page/clients_list/login") item=loop297}}
<tr{{assign "classAttributeTmp5" ""}}{{if $loop297@iteration % 2}}{{capture name="classAttributeTmp5" assign="classAttributeTmp5"}}affiliate_clients_row1{{/capture}}{{else}}{{capture name="classAttributeTmp5" assign="classAttributeTmp5"}}affiliate_clients_row2{{/capture}}{{/if}}  class="{{$classAttributeTmp5}}">
<td class="affiliate_clients_first">
<span class="affiliate_clients_name">
<a data-mobile-class="btn-small" href="{{$loop297->getItemEscape("@link")}}">{{$loop297->getItemEscape("@nr")}}</a></span></td>
<td>{{$loop297->getItemEscape("@date")}}</td></tr>{{/foreach}}</tbody></table></div></div>{{else}}
<div class="affiliate_clients_desc_null">
            {{$txt['59424::affiliate_clients_desc_null']}}
        </div>{{/if}}{{/if}}
<!--Klienci zarejestrowani z PP - Stronicowanie (affiliate_clients_paging, 59425.1)-->
{{if count( $xml->getList("page/clients_list/navigation/item"))}}{{capture name="lastItem" assign="lastItem"}}{{$xml->getItemEscape("page/clients_list/navigation/item[position() = last()]/@counter")}}{{/capture}}
<table class="affiliate_clients_paging">
<tr>
<td class="affiliate_clients_portions">{{if count( $xml->getList("page/clients_list/navigation/portions"))  gt (string) 1}}
<form class="affiliate_clients_sort" action="settings.php" id="affiliate_clients_sort">
<span class="affiliate_clients_portions">{{$txt['59425::affiliate_clients_001']}}</span>
<select class="affiliate_clients_portions" name="affiliate_portions" onchange="submit();">{{foreach from=$xml->getList("page/clients_list/navigation/portions") item=loop336}}
<option value="{{$loop336->getItemEscape("@counter")}}" {{if $loop336->getItem("@selected") == (string) 'true'}} selected="selected" {{/if}}>{{$loop336->getItemEscape("@counter")}}</option>{{/foreach}}</select>
<span class="affiliate_clients_portions">{{$txt['59425::affiliate_clients_002']}}</span></form>{{/if}}</td>
<td class="affiliate_clients_paging">
<div class="acs_paging_right">{{if count( $xml->getList("page/clients_list/navigation/item"))  gt (string) 1}}
<table class="acs_paging_sub">
<tr>
<td class="acs_paging_sub">{{if  $xml->getItem("page/clients_list/navigation/prev")}}
<a class="acs_paging_first" href="{{$xml->getItem("page/clients_list/navigation/path/@value")}}0">{{$txt['59425::affiliate_clients_003']|escape:'iai'}}</a>{{/if}}{{if !( $xml->getItem("page/clients_list/navigation/prev"))}}
<span class="acs_paging_first">{{$txt['59425::affiliate_clients_004']|escape:'iai'}}</span>{{/if}}{{if  $xml->getItem("page/clients_list/navigation/prev")}}
<a class="acs_paging_left" href="{{$xml->getItem("page/clients_list/navigation/path/@value")}}{{$xml->getItemEscape("page/clients_list/navigation/prev/@counter")}}">{{$txt['59425::affiliate_clients_005']|escape:'iai'}}</a>{{/if}}{{if !( $xml->getItem("page/clients_list/navigation/prev"))}}
<span class="acs_paging_left">{{$txt['59425::affiliate_clients_006']|escape:'iai'}}</span>{{/if}}{{foreach from=$xml->getList("page/clients_list/navigation/item") item=loop390}}{{if  $loop390->getItem("@gap")}}
<span class="acs_paging_gap">{{$txt['59425::affiliate_clients_007']}}</span>{{/if}}{{if !( $loop390->getItem("@type")  == (string) 'current') and !( $loop390->getItem("@gap"))}}
<a class="acs_paging" href="{{$loop390->getItem("../path/@value")}}{{$loop390->getItemEscape("@counter") - 1}}" {{if $loop390@iteration == (string) $loop390@total }} id="last_of_page"{{/if}}>{{$loop390->getItemEscape("@counter")}}</a>{{/if}}{{if  $loop390->getItem("@type")  == (string) 'current'}}
<span class="acs_paging_current">{{$loop390->getItemEscape("@counter")}}</span>{{/if}}{{if  $loop390@iteration   neq (string)  $loop390@total }}{{$txt['59425::affiliate_clients_008']}}{{/if}}{{/foreach}}{{if  $xml->getItem("page/clients_list/navigation/prev") and  $xml->getItem("page/clients_list/navigation/next")}}{{$txt['59425::affiliate_clients_008']}}{{/if}}{{if  $xml->getItem("page/clients_list/navigation/next")}}
<a class="acs_paging_right" href="{{$xml->getItem("page/clients_list/navigation/path/@value")}}{{$xml->getItemEscape("page/clients_list/navigation/next/@counter")}}">
                                        {{$txt['59425::affiliate_clients_009']}}
                                    </a>{{/if}}{{if !( $xml->getItem("page/clients_list/navigation/next"))}}
<span class="acs_paging_right">
                                        {{$txt['59425::affiliate_clients_010']}}
                                    </span>{{/if}}{{if  $xml->getItem("page/clients_list/navigation/next")}}
<a class="acs_paging_last" href="{{$xml->getItem("page/clients_list/navigation/path/@value")}}{{$xml->getItemEscape("$lastItem - 1")}}">{{$txt['59425::affiliate_clients_011']|escape:'iai'}}</a>{{/if}}{{if !( $xml->getItem("page/clients_list/navigation/next"))}}
<span class="acs_paging_last">{{$txt['59425::affiliate_clients_012']|escape:'iai'}}</span>{{/if}}</td></tr></table>{{/if}}</div></td></tr></table>{{/if}}</div></div>

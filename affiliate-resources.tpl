...
<span class="affiliate_resources_label">
    {{$txt['59900::affiliate_resources_label']}}
    </span></div>
<input type="hidden" id="payment_min"/>
<div class="affiliate_resources_wyplata">
    {{$txt['59900::affiliate_label_text']}} 
    
<span class="resources_total">{{$xml->getItemEscape("page/payments/@total_formatted")}}</span>{{if (0  gt (string)  $xml->getItem("page/payments/@total")) or ( $xml->getItem("page/payments/@total")  == (string) '0.00')}}
...
<div id="affiliate_warning1_sub">
                                {{$txt['59900::affiliate_resources_text22_new']}} 
                            </div></div>
<div id="affiliate_warning2" style="display:none;">
<div id="affiliate_warning2_sub">
                                {{$txt['59900::affiliate_resources_text33_new']}} 
                            </div></div>
<div class="clear"></div></div></div>
<div class="affiliate_resources_payment_type">{{if  $xml->getItem("page/client/@paymenttype")  == (string) 'cash'}}
<input type="radio" value="voucher" name="method" checked="checked"/> 
                            {{$txt['59900::affiliate_resources_text4']}}
                            
<input type="radio" value="cash" name="method"/> 
                            {{$txt['59900::affiliate_resources_text5']}}
                        {{else}}
                            {{$txt['59900::affiliate_resources_text6']}}
                        {{/if}}</div></form>{{/if}}</div>{{/if}}
<!--Środki do wykorzystania - lista wypłat (affiliate_resources_list, 59929.2)-->
{{if  $xml->getItem("page/resources")}}{{if count( $xml->getList("page/resources/resource"))}}
...
<thead>
<tr>
<th id="affiliate_resources_th1">{{$txt['59929::affiliate_resources_th1']}}</th>
<th id="affiliate_resources_th2">{{$txt['59929::affiliate_resources_th2']}}</th>
<th id="affiliate_resources_th3">{{$txt['59929::affiliate_resources_th3']}}</th>
<th id="affiliate_resources_th4">{{$txt['59929::affiliate_resources_th4']}}</th>
<th id="affiliate_resources_th5">{{$txt['59929::affiliate_resources_th5']}}</th>
<th id="affiliate_resources_th6">{{$txt['59929::affiliate_resources_th6']}}</th>
<th id="affiliate_resources_th7">{{$txt['59929::affiliate_resources_th7']}}</th></tr></thead>{{foreach from=$xml->getList("page/resources/resource") item=loop362}}
<tr{{assign "classAttributeTmp6" ""}}{{if $loop362@iteration % 2}}{{capture name="classAttributeTmp6" assign="classAttributeTmp6"}}affiliate_resources_row1{{/capture}}{{else}}{{capture name="classAttributeTmp6" assign="classAttributeTmp6"}}affiliate_resources_row2{{/capture}}{{/if}}  class="{{$classAttributeTmp6}}">
<td class="text1">{{$loop362->getItemEscape("@date")}}</td>
<td class="text">{{if  $loop362->getItem("@type")  == (string) 'cash'}}
                        {{$txt['59929::affiliate_resources_text_1']}}
                      {{else}}
                        {{$txt['59929::affiliate_resources_text_22']}}
                      {{/if}}</td>
<td class="text">
<span>{{$loop362->getItemEscape("@idkey")}}</span></td>
<td class="text">{{if  $loop362->getItem("@expire_date")  == (string) '0000-00-00'}}
                        {{$txt['59929::affiliate_resources_text_2']}}
                      {{elseif  $loop362->getItem("@expire_date")  == (string) ''}}
                        -
                      {{else}}{{$loop362->getItemEscape("@expire_date")}}{{/if}}</td>
<td class="text">{{$loop362->getItemEscape("@amount_formatted")}}</td>
<td class="text">{{$loop362->getItemEscape("@used_formatted")}}</td>
<td class="text">{{if  $loop362->getItem("@status")  == (string) 'n'}}
<span class="bl">{{$txt['59929::affiliate_resources_text_3']}}</span>{{elseif  $loop362->getItem("@status")  == (string) 'y'}}
<span class="g">{{$txt['59929::affiliate_resources_text_4']}}</span>{{elseif  $loop362->getItem("@status")  == (string) 'b'}}
<span class="r">{{$txt['59929::affiliate_resources_text_5']}}</span>{{elseif  $loop362->getItem("@status")  == (string) '0000-00-00'}}
<span class="bl">{{$txt['59929::affiliate_resources_text_66']}}</span>{{elseif  $loop362->getItem("@status")  == (string) 'Odrzucono'}}
<span class="r">{{$loop362->getItemEscape("@status")}}</span>{{else}}
<span class="g">{{$txt['59929::affiliate_resources_text_7']}}</span> ({{$loop362->getItemEscape("@status")}})
                      {{/if}}</td></tr>{{/foreach}}</table>{{else}}
<span class="affiliate_noresources">{{$txt['59929::affiliate_resources_text_6']}}</span>{{/if}}{{/if}}
<!--Środki do wykorzystania - stronicowanie listy wypł (affiliate_resources_paging, 59932.1)-->
{{if count( $xml->getList("page/navigation/item"))}}{{capture name="lastItem" assign="lastItem"}}{{$xml->getItemEscape("page/navigation/item[position() = last()]/@counter")}}{{/capture}}
<table class="affiliate_resources_paging">
<tr>
<td class="affiliate_resources_portions">{{if count( $xml->getList("page/navigation/portions"))  gt (string) 1}}
...
<select class="affiliate_resources_portions" name="affiliate_portions" onchange="submit();">{{foreach from=$xml->getList("page/navigation/portions") item=loop431}}
<option value="{{$loop431->getItemEscape("@counter")}}" {{if $loop431->getItem("@selected") == (string) 'true'}} selected="selected" {{/if}}>{{$loop431->getItemEscape("@counter")}}</option>{{/foreach}}</select>
<span class="affiliate_resources_portions">{{$txt['59932::affiliate_resources_002']}}</span></form>{{/if}}</td>
<td class="affiliate_resources_paging">
<div class="aclc_paging_right">{{if count( $xml->getList("page/navigation/item"))  gt (string) 1}}
<table class="aclc_paging_sub">
<tr>
<td class="aclc_paging_sub">{{if  $xml->getItem("page/navigation/prev")}}
<a class="aclc_paging_first" href="{{$xml->getItem("page/navigation/path/@value")}}0">{{$txt['59932::affiliate_resources_003']|escape:'iai'}}</a>{{/if}}{{if !( $xml->getItem("page/navigation/prev"))}}
<span class="aclc_paging_first">{{$txt['59932::affiliate_resources_004']|escape:'iai'}}</span>{{/if}}{{if  $xml->getItem("page/navigation/prev")}}
<a class="aclc_paging_left" href="{{$xml->getItem("page/navigation/path/@value")}}{{$xml->getItemEscape("page/navigation/prev/@counter")}}">{{$txt['59932::affiliate_resources_005']|escape:'iai'}}</a>{{/if}}{{if !( $xml->getItem("page/navigation/prev"))}}
<span class="aclc_paging_left">{{$txt['59932::affiliate_resources_006']|escape:'iai'}}</span>{{/if}}{{foreach from=$xml->getList("page/navigation/item") item=loop485}}{{if  $loop485->getItem("@gap")}}
<span class="aclc_paging_gap">{{$txt['59932::affiliate_resources_007']}}</span>{{/if}}{{if !( $loop485->getItem("@type")  == (string) 'current') and !( $loop485->getItem("@gap"))}}
<a class="aclc_paging" href="{{$loop485->getItem("../path/@value")}}{{$loop485->getItemEscape("@counter") - 1}}" {{if $loop485@iteration == (string) $loop485@total }} id="last_of_page"{{/if}}>{{$loop485->getItemEscape("@counter")}}</a>{{/if}}{{if  $loop485->getItem("@type")  == (string) 'current'}}
<span class="aclc_paging_current">{{$loop485->getItemEscape("@counter")}}</span>{{/if}}{{if  $loop485@iteration   neq (string)  $loop485@total }}{{$txt['59932::affiliate_resources_008']}}{{/if}}{{/foreach}}{{if  $xml->getItem("page/navigation/prev") and  $xml->getItem("page/navigation/next")}}{{$txt['59932::affiliate_resources_008']}}{{/if}}{{if  $xml->getItem("page/navigation/next")}}
<a class="aclc_paging_right" href="{{$xml->getItem("page/navigation/path/@value")}}{{$xml->getItemEscape("page/navigation/next/@counter")}}">
                                        {{$txt['59932::affiliate_resources_009']}}
                                    </a>{{/if}}{{if !( $xml->getItem("page/navigation/next"))}}
<span class="aclc_paging_right">
                                        {{$txt['59932::affiliate_resources_010']}}
                                    </span>{{/if}}{{if  $xml->getItem("page/navigation/next")}}
<a class="aclc_paging_last" href="{{$xml->getItem("page/navigation/path/@value")}}{{$xml->getItemEscape("$lastItem - 1")}}">{{$txt['59932::affiliate_resources_011']|escape:'iai'}}</a>{{/if}}{{if !( $xml->getItem("page/navigation/next"))}}
<span class="aclc_paging_last">{{$txt['59932::affiliate_resources_012']|escape:'iai'}}</span>{{/if}}</td></tr></table>{{/if}}</div></td></tr></table>{{/if}}</div></div>

...
<i></i>
                {{$txt['69376::step6_txt']}}
            </p></div></div>{{if count( $xml->getList("page/pickups/pickup"))  gt (string) 1}}
<h1 class="big_label return_label">{{$txt['69376::51900_00634stock']}}</h1>{{else}}
<h1 class="big_label return_label">{{$txt['69376::51900_00634stock1']}}</h1>{{/if}}
<form class="pickup_return_list_form" method="get">{{if count( $xml->getList("page/pickups/pickup"))  gt (string) 1}}{{foreach from=$xml->getList("page/pickups/pickup") item=loop352}}
<div{{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}pickupl_return{{/capture}}{{if $loop352->getItem("@checked") == (string) 'true'}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}pickupl_return active{{/capture}}{{/if}} id="ps_{{$loop352->getItemEscape("@value")}}"  class="{{$classAttributeTmp4}}">
<input style="display:none;" type="radio" name="stock" {{if $loop352->getItem("@checked") == (string) 'true'}} checked="checked"{{/if}} value="{{$loop352->getItemEscape("id")}}"></input>
<div class="pickupl_return_sub">
<div class="pickupl_return_header">
<span class="pickupl_return_name">{{$loop352->getItemEscape("name")}}</span></div>
<div class="pickupl_return_address">
<span class="pickup_return_address" style="display:none">{{$loop352->getItemEscape("address")}}</span>{{$loop352->getItemEscape("address")}}
<span class="pickupl_return_city">{{$loop352->getItemEscape("zipcode")}}{{literal}} {{/literal}}{{$loop352->getItemEscape("city")}}</span></div>
<div class="pickupl_return_link">
<a target="_blank" href="{{$loop352->getItemEscape("link")}}" title="{{$txt['69376::pickupl_return_linktitle']}}">
                                                {{$txt['69376::pickupl_return_link']}}
                                            </a></div>{{if  $loop352->getItem("info")}}
<div class="pickupl_return_info">{{$loop352->getItemEscape("info")}}</div>{{/if}}</div></div>{{/foreach}}
<script>
                            ...
                            </script>{{else}}{{foreach from=$xml->getList("page/pickups/pickup") item=loop401}}
<div class="pickupl_return_one" id="ps_{{$loop401->getItemEscape("@value")}}">
<input style="display:none;" type="radio" name="stock" checked="checked" value="{{$loop401->getItemEscape("id")}}"></input>
<div class="pickupl_return_one_sub">
<div class="pickupl_return_one_link">
<a target="_blank" data-tablet-class="btn-small" data-mobile-class="btn-small" href="{{$loop401->getItemEscape("link")}}" title="{{$txt['69376::pickupl_return_one_linktitle']}}">
                                                {{$txt['69376::pickupl_return_one_link']}}
                                            </a></div>
<div class="pickupl_return_one_header">
<span class="pickupl_return_one_name">{{$loop401->getItemEscape("name")}}</span></div>
<div class="pickupl_return_one_address">{{$loop401->getItemEscape("address")}}{{literal}} {{/literal}}
<span class="pickupl_return_one_city">{{$loop401->getItemEscape("zipcode")}}{{literal}} {{/literal}}{{$loop401->getItemEscape("city")}}</span></div>{{if  $loop401->getItem("info")}}
<div class="pickupl_return_one_info">{{$loop401->getItemEscape("info")}}</div>{{/if}}</div></div>{{/foreach}}{{/if}}

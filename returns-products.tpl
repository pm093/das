...
<h1 class="big_label return_label">{{$txt['69362::51900_006111']}}</h1>{{capture name="return_with_special_offer" assign="return_with_special_offer"}}{{$txt['69362::57649_return_with_special_offer']}}{{/capture}}{{capture name="required_from_special_offer" assign="required_from_special_offer"}}{{$txt['69362::57649_required_from_special_offer']}}{{/capture}}{{if $return_with_special_offer  neq (string) '' and  $xml->getItem("/shop/page/products/@return_with_special_offer")  == (string) 'true'}}
<div class="menu_messages_message menu_messages_return_with_specia">
<div>{{$return_with_special_offer}}</div></div>{{/if}}{{if  $xml->getItem("page/products")}}{{capture name="setIconLarge" assign="setIconLarge"}}{{$txt['69362::57649_setIconLarge']}}{{/capture}}{{capture name="wholesalerPrice" assign="wholesalerPrice"}}{{$txt['69362::57649_wholesalerPrice']}}{{/capture}}{{capture name="hideLimit" assign="hideLimit"}}{{$txt['69362::57649_hideLimit']}}{{/capture}}{{capture name="allowReturnInfo" assign="allowReturnInfo"}}{{$txt['69362::57649_allowReturn']}}{{/capture}}{{capture name="allowReturnInfoPoint" assign="allowReturnInfoPoint"}}{{$txt['69362::57649_allowReturnInfoPoint']}}{{/capture}}{{capture name="allowReturnInfoGratis" assign="allowReturnInfoGratis"}}{{$txt['69362::57649_allowReturnInfoGratis']}}{{/capture}}
...
<th colspan="2">
                                {{$txt['69362::50001_03']}}
                            </th>{{if  $xml->getItem("/shop/page/products/product/@required_from_special_offer")  == (string) 'true' and $required_from_special_offer  neq (string) ''}}
<th>
                                  {{$txt['69362::50001_equired_from_special_offer']}}
                              </th>{{/if}}
<th>
                                {{$txt['69362::50001_05']}}
                            </th>
...
<th>
                                {{$txt['69362::50001_08a']}}
                            </th></tr>{{foreach from=$xml->getList("page/products/product") item=loop377}}
<tr class="productslist_item">
<td class="productslist_product_photo">
<a href="{{$loop377->getItemEscape("@link")}}">
<img {{if $setIconLarge}} src="{{$loop377->getItemEscape("icon")}}" {{else}} src="{{$loop377->getItemEscape("iconsmall")}}" {{/if}} alt="{{$loop377->getItem("name/text()")}}"></img></a></td>
<td class="productslist_product_name">
<h3>
<a href="{{$loop377->getItemEscape("@link")}}">{{$loop377->getItem("name/text()")}}</a></h3>{{if !( $loop377->getItem("@barcode")  == (string)'')}}
<div class="productslist_product_barcode">
<span>
                                                {{$txt['69362::50001_99']}}
                                            </span>{{$loop377->getItemEscape("@barcode")}}</div>{{/if}}{{if !( $loop377->getItem("size/@type") == (string)'uniw')}}
<div class="productslist_product_size">
<span>
                                                {{$txt['69362::50001_004']}}
                                            </span>{{$loop377->getItemEscape("size/@description")}}</div>{{/if}}{{if ( $loop377->getItem("version")) and (!( $loop377->getItem("version/@product_name") == (string)''))}}
<div class="productslist_product_version">
<span>{{$loop377->getItemEscape("version/@name")}}:
                                            </span>{{$loop377->getItemEscape("version/@product_name")}}</div>{{/if}}{{if $allowReturnInfo and  $loop377->getItem("@allowReturn") == (string)'0'}}
<div{{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}productslist_product_info{{/capture}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}productslist_product_info visible-phone{{/capture}}  class="{{$classAttributeTmp4}}">{{$allowReturnInfo}}</div>{{/if}}{{if  $loop377->getItem("/shop/page/products/product/@required_from_special_offer")  == (string) 'true' and $required_from_special_offer  neq (string) ''}}{{if  $loop377->getItem("@required_from_special_offer")  == (string) 'true'}}
<div{{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}productslist_product_info{{/capture}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}productslist_product_info visible-phone{{/capture}}  class="{{$classAttributeTmp4}}">
                                            {{$txt['69362::57649_required_from_special_offer']}}
                                        </div>{{/if}}{{/if}}</td>{{if  $loop377->getItem("/shop/page/products/product/@required_from_special_offer")  == (string) 'true' and $required_from_special_offer  neq (string) ''}}
<td{{assign "classAttributeTmp6" ""}}{{capture name="classAttributeTmp6" assign="classAttributeTmp6"}}productslist_product_special_offer{{/capture}}{{capture name="classAttributeTmp6" assign="classAttributeTmp6"}}productslist_product_special_offer hidden-phone{{/capture}}  class="{{$classAttributeTmp6}}">{{if  $loop377->getItem("@required_from_special_offer")  == (string) 'true'}}
<div class="productslist_product_info">
                                        {{$txt['69362::57649_required_from_special_offer']}}
                                        </div>{{/if}}</td>{{/if}}
<td class="productslist_product_prices">{{if  $loop377->getItem("size/@ordered_points")}}
<del>{{if ( $loop377->getItem("/shop/basket/@wholesaler")  == (string) 'true') and !($wholesalerPrice)}}{{$loop377->getItemEscape("price/@price_net_formatted")}}{{else}}{{$loop377->getItemEscape("price/@price_formatted")}}{{/if}}</del>
<span>{{$loop377->getItemEscape("price/@points")}}{{$txt['69362::55925_pkt']}}
                                            </span>{{else}}{{if  $loop377->getItem("price/@beforerebate")}}
<del>{{$loop377->getItemEscape("price/@beforerebate_formatted")}}</del>{{/if}}
<span>{{if ( $loop377->getItem("/shop/basket/@wholesaler")  == (string) 'true') and !($wholesalerPrice)}}{{$loop377->getItemEscape("price/@price_net_formatted")}}{{else}}{{$loop377->getItemEscape("price/@price_formatted")}}{{/if}}</span>{{/if}}</td>
<td class="productslist_product_quantity">{{if $allowReturnInfoPoint and  $loop377->getItem("@allowReturn") == (string)'0' and  $loop377->getItem("@for_points") == (string)'true'}}
                                         colspan="2"
                                        
<div class="productslist_product_info">{{$allowReturnInfoPoint}}</div>{{elseif $allowReturnInfoGratis and  $loop377->getItem("@allowReturn") == (string)'0' and  $loop377->getItem("@gratis") == (string)'t'}}
                                         colspan="2"
                                        
<div class="productslist_product_info">{{$allowReturnInfoGratis}}</div>{{elseif $allowReturnInfo and  $loop377->getItem("@allowReturn") == (string)'0'}}
                                         colspan="2"
                                        
<div class="productslist_product_info">{{$allowReturnInfo}}</div>{{else}}
<div>{{if  $loop377->getItem("change_group/@id")}}
                                             class="productslist_product_quantity productslist_product_quantity_change"
                                            
<input type="hidden" name="product[{{$loop377@iteration}}]" value="{{$loop377->getItemEscape("@id")}}"></input>
<input type="hidden" name="size[{{$loop377@iteration}}]" value="{{$loop377->getItemEscape("size/@type")}}"></input>{{if  $loop377->getItem("additional/@hash")}}
<input type="hidden" name="additional[{{$loop377@iteration}}]" value="{{$loop377->getItemEscape("additional/@hash")}}"></input>{{/if}}{{if  $loop377->getItem("size/@ordered_points")}}
<input type="hidden" value="1" name="forpoints[{{$loop377@iteration}}]"></input>{{/if}}
<input{{assign "valueAttributeTmp7" ""}}  type="hidden" name="set_quantity[{{$loop377@iteration}}]" {{if $loop377->getItem("size/@ordered_points")}}{{capture name="valueAttributeTmp7" assign="valueAttributeTmp7"}}{{$loop377->getItemEscape("size/@ordered_points")}}{{/capture}}{{else}}{{capture name="valueAttributeTmp7" assign="valueAttributeTmp7"}}{{$loop377->getItemEscape("size/@ordered")}}{{/capture}}{{/if}}  value="{{$valueAttributeTmp7}}"></input>
<strong>{{if  $loop377->getItem("size/@ordered_points")}}{{$loop377->getItemEscape("size/@ordered_points")}}{{else}}{{$loop377->getItemEscape("size/@ordered")}}{{/if}}
<span>{{if !( $loop377->getItem("size/@ordered")  gt (string) 1)}}{{$loop377->getItemEscape("size/@unit_singular")}}{{else}}{{$loop377->getItemEscape("size/@unit")}}{{/if}}</span></strong>{{if  $loop377->getItem("change_group/@changelink")}}
<a class="productslist_change_product" href="{{$loop377->getItemEscape("change_group/@changelink")}}">
                                                    {{$txt['69362::50001_99wy']}}
                                                </a>{{/if}}{{else}}
<input type="hidden" name="size[{{$loop377@iteration}}]" value="{{$loop377->getItemEscape("size/@type")}}"></input>{{if  $loop377->getItem("additional/@hash")}}
<input type="hidden" name="additional[{{$loop377@iteration}}]" value="{{$loop377->getItemEscape("additional/@hash")}}"></input>{{/if}}{{if  $loop377->getItem("size/@ordered_points")}}
<input type="hidden" value="1" name="forpoints[{{$loop377@iteration}}]"></input>{{/if}}
...
<input type="text" name="set_quantity[{{$loop377@iteration}}]" value="{{$loop377->getItemEscape("@selected_quantity")}}"></input>
<input type="hidden" name="data_amount" value="{{$loop377->getItemEscape("@quantity")}}"></input>
<input type="hidden" name="data_unit_sellby" value="{{$loop377->getItemEscape("size/@unit_sellby")}}"></input>
<input type="hidden" name="data_unit" value="{{$loop377->getItemEscape("size/@unit")}}"></input>
<input type="hidden" name="data_unit_precision" value="{{$loop377->getItemEscape("size/@unit_precision")}}"></input>
<a href="#addQuantity" class="addQuantity">{{$txt['69362::50001_082']}}</a>
<i class="productslist_product_unit">{{if !( $loop377->getItem("size/@ordered")  gt (string) 1)}}{{$loop377->getItemEscape("size/@unit_singular")}}{{else}}{{$loop377->getItemEscape("size/@unit")}}{{/if}}</i></span>{{if !( $loop377->getItem("size/@unit_sellby")  == (string) '1')}}
<div class="productslist_product_sellby_info">{{$txt['69362::50001_900bb_xxx']}}
                                                {{$loop377->getItemEscape("size/@unit_sellby")}}{{literal}} {{/literal}}{{if !( $loop377->getItem("size/@ordered")  gt (string) 1)}}{{$loop377->getItemEscape("size/@unit_singular")}}{{else}}{{$loop377->getItemEscape("size/@unit")}}{{/if}}</div>{{/if}}{{/if}}</div>{{/if}}</td>
<td class="productslist_product_action" {{if $allowReturnInfo and $loop377->getItem("@allowReturn") == (string)'0'}} style="display:none;" {{/if}}>
<input type="checkbox" name="product[{{$loop377@iteration}}]" value="{{$loop377->getItemEscape("@id")}}" {{if $loop377->getItem("@checked") == (string) 'true' or $loop377->getItem("@selected_quantity") gt (string) 0}} checked="checked"{{/if}}></input></td></tr>{{/foreach}}{{if !($allowReturnInfo) or ($allowReturnInfo and (count( $xml->getList("/shop/page/products/product")) - count( $xml->getList("/shop/page/products/product[@allowReturn='0']")))  gt (string) 0)}}
<tr class="productslist_tools_bottom">
<td colspan="5" {{if $xml->getItem("/shop/page/products/product/@required_from_special_offer") == (string) 'true' and $required_from_special_offer neq (string) ''}} colspan="6" {{/if}}>{{if !($allowReturnInfo) or ($allowReturnInfo and (count( $xml->getList("/shop/page/products/product")) - count( $xml->getList("/shop/page/products/product[@allowReturn='0']")))  gt (string) 1)}}
<a href="#selectall" class="btn-small select_all">{{$txt['69362::50001_10']}}</a>{{/if}}
<label class="return_product">
<input type="checkbox" class="return_product" {{if $xml->getItem("/shop/page/remarks")}} checked="checked"{{/if}}></input>
                                        {{$txt['69362::50001_10as']}}
                                    </label></td></tr>{{/if}}</table>
<textarea name="return_comment" class="return_comment"{{if $xml->getItem("/shop/page/remarks")}}{{$xml->getItem("/shop/page/remarks")}}{{else}} style="display:none"{{/if}}></textarea></div>
...
<a class="btn" href="{{$xml->getItemEscape("page/backlink/@url")}}">
                                        {{$txt['69362::order2_button_back']}}
                                    </a>{{/if}}</div>{{if !($allowReturnInfo) or ($allowReturnInfo and (count( $xml->getList("/shop/page/products/product")) - count( $xml->getList("/shop/page/products/product[@allowReturn='0']")))  gt (string) 0)}}
<div class="btn_wrapper">
<button id="return_go_next" type="submit" class="btn-large btn_arrow">
                                    {{$txt['69362::50001_10a']}}
                                </button></div>{{/if}}</div></form>{{/if}}</div>
<script>
    goNext = function(){
        var flaga = false;

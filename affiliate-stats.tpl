...
<!--Przekierowania - lista (affiliate_stats_list, 59250.1)-->
{{if  $xml->getItem("page/affiliate_statistics")}}
<div{{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}affiliate_stats_label{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}big_label affiliate_stats_label{{/capture}}  class="{{$classAttributeTmp3}}">
<span class="affiliate_stats_label">
        {{$txt['59250::affiliate_stats_label']}}
    </span></div>
<div class="affiliate_stats_desc">
<p class="affiliate_stats_desc">{{$txt['59250::affiliate_stats_desc']}}</p></div>
<div class="affiliate_stats">
<div class="affiliate_stats_sub">
<table{{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}affiliate_stats{{/capture}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}ui-responsive table-stroke ui-table ui-table-reflow affiliate_stats{{/capture}}  class="{{$classAttributeTmp4}}">
<thead>
<tr>
<th>{{$txt['59250::affiliate_stats_th1']}}</th>
<th>{{$txt['59250::affiliate_stats_th2']}}</th>
<th>{{$txt['59250::affiliate_stats_th3']}}</th>
<th>{{$txt['59250::affiliate_stats_th4']}}</th>
<th>{{$txt['59250::affiliate_stats_th5']}}</th></tr></thead>
<tbody>{{foreach from=$xml->getList("page/affiliate_statistics") item=loop297}}
<tr>{{foreach from=$loop297->getList("item") item=loop300}}
<td {{if $loop300->getItem("@name") == (string) 'jscode'}} class="affiliate_stats_last"{{/if}}>{{$loop300->getItemEscape("@count")}}</td>{{/foreach}}</tr>{{/foreach}}</tbody></table></div></div>{{/if}}</div></div>
